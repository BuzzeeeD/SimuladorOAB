...
document.addEventListener('DOMContentLoaded', () => {
    ...
    new Chart(ctxProgress, {
        type: 'doughnut',
        data: {
            labels: ['Direito Penal', 'Direito Civil', 'Raciocínio Lógico'],
            datasets: [{
                data: [65, 75, 85],
                backgroundColor: ['#FFCA28', '#00509E', '#FF5722'],
                hoverOffset: 10
            }]
        },
        options: {
    ...
    new Chart(ctxPerformance, {
        type: 'bar',
    data: {
        labels: ['Simulado 1', 'Simulado 2', 'Simulado 3', 'Simulado 4', 'Simulado 5', 'Simulado 6'],
        datasets: [{
            label: 'Desempenho (%)',
            data: [70, 80, 90, 85, 75, 95], // Adicione os dados para os novos simulados
            backgroundColor: ['#FFCA28', '#00509E', '#FF5722', '#4CAF50', '#9C27B0', '#FFC107'],
            borderWidth: 1,
            borderColor: '#ffffff',
        }],
    },
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    top10Users.forEach((user, index) => {
        ...
        // Gerar o handle (@nomedeusuario)
        const usernameHandle = `@${user.username.toLowerCase().replace(/\s+/g, '').replace(/[^\w]/g, '')}`;

        // Preencher a linha da tabela
        row.innerHTML = `
            <td>${index + 1}</td>
            <td><img src="${patenteImage}" alt="${patenteName}" class="patente-icon" style="width: 30px;"></td>
