...
//FUNÇÕES DAS ABAS EXPANSIVAS
function togglePurchases() {
    const content = document.querySelector('.purchases-content');
    const arrow = document.querySelector('.profile-purchases .toggle-arrow');

    if (content.style.display === 'none') {
        content.style.display = 'block';
        arrow.classList.add('expanded'); // Adiciona a classe para girar a seta
    } else {
        content.style.display = 'none';
        arrow.classList.remove('expanded'); // Remove a classe para voltar à posição original
    }
}

function toggleNotifications() {
    const content = document.querySelector('.notifications-content');
    const arrow = document.querySelector('.notifications-section .toggle-arrow');
...
function toggleRecentActivities() {
    const content = document.querySelector('.recent-activities-list');
    const arrow = document.querySelector('.recent-activities .toggle-arrow');
...
function toggleObjectives() {
    ...
    const arrow = document.querySelector('.objectives-section .toggle-arrow');

    if (content.style.display === 'block') {
        content.style.display = 'none';
        arrow.classList.remove('expanded'); // Remove a classe para girar a seta de volta
    } else {
        content.style.display = 'block';
        arrow.classList.add('expanded'); // Adiciona a classe para girar a seta
    }
...
function toggleRecommendations() {
    ...
    const arrow = document.querySelector('.recommendations-section .toggle-arrow');
    ...
    if (content.style.display === 'none' || content.style.display === '') {
        ...
        content.style.display = 'none';
        arrow.classList.remove('expanded'); // Recolhe o conteúdo e seta
    }
}

//carousel de desafios
let currentIndex = 0;

function scrollCarousel(direction) {
    const wrapper = document.querySelector('.carousel-wrapper');
    const track = document.querySelector('.carousel-track');
    const cardWidth = track.children[0].offsetWidth + 10; // Inclui margem (gap)
    const visibleCards = Math.floor(wrapper.offsetWidth / cardWidth); // Cartões visíveis
    const maxIndex = track.children.length - visibleCards;

    currentIndex += direction;
    currentIndex = Math.max(0, Math.min(currentIndex, maxIndex)); // Garante o índice dentro do intervalo válido
...
const track = document.querySelector('.carousel-track');
const wrapper = document.querySelector('.carousel-wrapper');
const cardWidth = track.children[0].offsetWidth + 10; // Inclui margem entre os cards
const maxTranslate = -cardWidth * (track.children.length - Math.floor(wrapper.offsetWidth / cardWidth));
...
// Move o carousel conforme o cursor/dedo
function dragMove(e) {
...
function dragEnd() {
    ...
    if (movedBy < -cardWidth / 3 && currentTranslate > maxTranslate) {
        currentIndex += 1;
    } else if (movedBy > cardWidth / 3 && currentTranslate < 0) {
        currentIndex -= 1;
    }
