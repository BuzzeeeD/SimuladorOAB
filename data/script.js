...
window.addEventListener('scroll', function() {
    ...
    const visibleQuestions = document.querySelectorAll('.question:not([style*="display: none"])').length > 0;

    // Oculta o question-navigator quando ele ultrapassa a linha inferior do header
    if (scrollPosition > headerBottom && visibleQuestions) {
        questionNavigator.style.display = 'flex'; // Exibe o question-navigator
    } else {
        questionNavigator.style.display = 'none'; // Oculta o question-navigator
    }
});
// Atualiza o campo de número da questão ao rolar a página
window.addEventListener('scroll', function() {
    let visibleQuestions = document.querySelectorAll('.question:not([style*="display: none"])'); // Seleciona apenas as questões visíveis
    let currentQuestionNumber = null;

    visibleQuestions.forEach((question) => {
        const rect = question.getBoundingClientRect();
        if (rect.top >= 0 && rect.top <= window.innerHeight / 2) {
            currentQuestionNumber = question.getAttribute('data-navigation-id');  // Pega o valor do data-navigation-id
        }
    });
...
function populateExamSelect() {
    ...
    // Adiciona o evento ao checkbox para disparar a função sempre que for marcado/desmarcado
document.getElementById('hide-anuladas-checkbox').addEventListener('change', toggleAnuladas);
// Função para ocultar/exibir questões anuladas
function toggleAnuladas() {
    const hideAnuladas = document.getElementById('hide-anuladas-checkbox').checked; // Verifica o estado do checkbox
    const questions = document.querySelectorAll('.question'); // Seleciona todas as questões exibidas

    // Percorre todas as questões
    questions.forEach(question => {
        const isAnulada = question.style.backgroundColor === 'rgba(255, 248, 179, 0.8)'; // Verifica se a questão foi marcada como anulada
        
        // Ocultar ou exibir a questão com base no estado do checkbox
        if (isAnulada) {
            question.style.display = hideAnuladas ? 'none' : 'block'; // Oculta ou exibe a questão
        }
    });
}
    ...
    defaultOption.value = "";
    defaultOption.textContent = `Escolha o Exame`;
...
examSelect.addEventListener('change', function() {
   ...
   function displayQuestions(examNumber) {
    ...
    // Mostrar o container do checkbox apenas se houver questões
...
`;
   ...
   function displayDescriptionContainer(totalQuestionsAll) {
    ...
    descriptionText.innerHTML = `
        ...
        <p style="font-size: 16px; line-height: 1.6;">
            Sinta-se à vontade para contribuir com qualquer valor. <strong>Basta escanear ou clicar no QR code abaixo.</strong> Toda ajuda faz a diferença!
        </p>
    `;
    ...
    // Criar o botão "Análise de Dados"
    const dataAnalysisButton = document.createElement('button');
    dataAnalysisButton.id = 'data-analysis-btn';
    dataAnalysisButton.textContent = 'Análise de Dados';
    dataAnalysisButton.style.marginTop = '20px'; // Adicionar margem superior para o espaçamento

    // Adicionar evento de clique para redirecionar para "analisededados.html"
    dataAnalysisButton.addEventListener('click', function() {
        window.location.href = 'analisededados.html'; // Redirecionar para a página de análise de dados
    });

    // Adicionar o texto, o QR code e o botão ao container
    descriptionDiv.appendChild(descriptionText);
    descriptionDiv.appendChild(dataAnalysisButton); // Adiciona o botão no container
...
function populateDisciplinaSelect() {
    ...
    // Ordenar as disciplinas em ordem decrescente conforme a quantidade de questões
    const sortedDisciplinas = Object.keys(disciplinasContagem).sort((a, b) => disciplinasContagem[b] - disciplinasContagem[a]);
    ...
    disciplinaSelect.addEventListener('change', function() {
        ...
        // Se "Escolha a Disciplina" for selecionado (ou nenhuma)
        if (!selectedDisciplina) {
            layoutContainer.innerHTML = '';  // Limpar as questões
            gabaritoBtn.style.display = 'none';  // Ocultar o botão Gabarito
            finalizarProvaBtn.style.display = 'none'; // Ocultar o botão Finalizar Prova
            const checkboxContainer = document.querySelector('.checkbox-container');
            checkboxContainer.style.display = 'none';
            displayDescriptionContainer();  // Exibir o container de descrição
        } else {
            // Exibir as questões filtradas se uma disciplina válida for escolhida
            displayFilteredQuestions(selectedDisciplina);
            gabaritoBtn.style.display = 'block';  // Mostrar o botão Gabarito
            finalizarProvaBtn.style.display = 'block'; // Mostrar o botão Finalizar Prova
            // Mostrar o container do checkbox apenas se houver questões
            const checkboxContainer = document.querySelector('.checkbox-container');
    ...
    // Função para contar as questões anuladas na exibição
function contarQuestoesAnuladas() {
    const questions = document.querySelectorAll('.question'); // Seleciona todas as questões exibidas
    let anuladasCount = 0;

    questions.forEach(question => {
        const isAnulada = question.style.backgroundColor === 'rgba(255, 248, 179, 0.8)'; // Verifica se a questão está marcada como anulada
        if (isAnulada) {
            anuladasCount++;
        }
    });

    // Atualiza o contador de questões anuladas ao lado do checkbox
    document.getElementById('anuladas-count').textContent = `(${anuladasCount})`;
}
    ...
    function displayFilteredQuestions(disciplina) {
        ...
        // Mostrar o checkbox-container
        const checkboxContainer = document.querySelector('.checkbox-container');
        checkboxContainer.style.display = 'flex';  // Garante que o checkbox seja exibido quando as questões forem carregadas
        ...
        // Atualizar o número de questões anuladas
        contarQuestoesAnuladas();
    }
